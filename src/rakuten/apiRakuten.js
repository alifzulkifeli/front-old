...
export const R_Search = () => {
    if (localStorage.getItem("R_List") === null) {
        localStorage.setItem('R_List', JSON.stringify([]));
    }
    if (localStorage.getItem("R_Search") === null) {
        localStorage.setItem('R_Search', JSON.stringify(''));
    }
    let seachHistory = localStorage.getItem('R_Search').replace(/['"]+/g, '');
    let seachHistory2 = localStorage.getItem('R_List').replace(/['"]+/g, '');
    ...
    localStorage.setItem('R_Search', JSON.stringify(params.search));
    
      return fetch(`${API}/rakuten/search?page=${page || 1}&min_price=${Math.trunc(minPrice/0.038)}&max_price=${Math.trunc(maxPrice/0.038)}&search=${search}`, {
          method: "GET"
      })
          .then(response => {
            ...
            list({ search: search || undefined, category: category }).then(
                response => {
                    if (response.error) {
                        ...
                        localStorage.setItem('R_List', JSON.stringify(response));
    ...
    const showLoading = () =>
        ...
        let last = JSON.parse(localStorage.getItem('R_List')) 
    return (
        <div className="row">
...
export const Item1 = ({name}) => {
    ...
    const list = () => {
      return fetch(`${API}/mercari/search?search=rakuten`, {
          method: "GET"
      })
          .then(response => {
              return response.json();
